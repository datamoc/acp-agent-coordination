...
import { OPS } from "./ops.generated.js";
import { TokenProvider } from "./oidc.js";
import { LocalTransport, RemoteTransport } from "./transport.js";
...
export function tokenSource(env = process.env) {
    if (env.COORD_TOKEN)
        return env.COORD_TOKEN;
    if (env.COORD_OIDC_ISSUER || env.COORD_OIDC_TOKEN_URL)
        return TokenProvider.fromEnv(env);
    return null;
}
...
export class CoordClient {
    ...
    async a2a(method, params) {
        if (!this.transport.a2a)
            throw new CoordError("a2a_unsupported", "A2A needs a coord server (COORD_SERVER), not local mode");
    ...
    async agentCard() {
        ...
        if (!t.agentCard)
            throw new CoordError("a2a_unsupported", "the agent card needs a coord server (COORD_SERVER)");
        return t.agentCard();
    }
    async call(op, args) {
        if (!(op in OPS))
            throw new CoordError("bad_op", `unknown op ${op}`);
        const clean = Object.fromEntries(Object.entries(args).filter(([, v]) => v !== undefined));
        const env = await this.transport.send(op, clean);
        if (!env.ok)
            throw new CoordError(env.error ?? "error", env.message ?? "", env.data ?? null);
        return env.result;
    }
}
