...
function checkFamily(family: string | undefined): void {
...
const num = (v: string | null | undefined, what: string): number | null => {
  if (v === null || v === undefined || v === "") return null;
  const n = Number(v);
  if (!Number.isFinite(n)) throw new UsageError(`argument ${what}: invalid number: '${v}'`);
  return n;
};
...
export async function run(path: string[], a: NS, c: CoordClient): Promise<[string, any, number]> {
  ...
  const maybeS = (): string | null => { try { return S(); } catch { return null; } };
  ...
  switch (cmd) {
    ...
    case "events": {
      ...
      if (!a.follow) return [cmd, await call("events", { after: a.after, project, session: maybeS() }), 0];
      const stop = new AbortController();
      process.once("SIGINT", () => stop.abort());
      const t = c.transport;
      const follow = t.follow ? t.follow.bind(t) : (p: string | null, af: number, on: (e: CoordEvent) => void, s: AbortSignal) =>
        pollEvents(t, p, af, on, s, 2000, maybeS());
