...
type Op = { tag: " " | "-" | "+"; a: number; b: number }; // a/b: index in old/new (for "+", a = position)
...
function script(a: string[], b: string[]): Op[] {
  ...
  for (let d = trace.length - 1; d >= 0; d--) {
    ...
    while (x > px && y > py) { x--; y--; ops.push({ tag: " ", a: x, b: y }); }
    if (d > 0) {
      if (x === px) { y--; ops.push({ tag: "+", a: x, b: y }); } else { x--; ops.push({ tag: "-", a: x, b: y }); }
    }
  }
  return ops.reverse();
}
...
export function unifiedDiff(oldText: string, newText: string, context = 3, names = ["a", "b"]): string {
  ...
  const key = (xs: string[], noNl: boolean) => (noNl ? [...xs.slice(0, -1), xs[xs.length - 1] + "\u0000"] : xs);
  ...
  while (i < ops.length) {
    ...
    for (;;) {                                   // extend over changes separated by <= 2*context equal lines
      while (end < ops.length && ops[end].tag !== " ") end++;
      let eq = end;
      while (eq < ops.length && ops[eq].tag === " ") eq++;
      if (eq < ops.length && eq - end <= 2 * context) { end = eq; continue; }
    ...
    for (const o of hunk) {
      ...
      if (o.tag === " " ? lastOld || lastNew : o.tag === "-" ? lastOld : lastNew) out.push("\\ No newline at end of file");
    }
    i = end;
  }
  return out.join("\n") + "\n";
