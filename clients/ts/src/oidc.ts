...
import { httpRequest, type TokenSource } from "./transport.js";
...
const stateDirDefault = (env: NodeJS.ProcessEnv) => join(env.XDG_CONFIG_HOME || join(homedir(), ".config"), "coord", "oidc");
const sleep = (ms: number): Promise<void> => new Promise((r) => setTimeout(r, ms));

export class TokenProvider implements TokenSource {
  readonly stateFile: string;
  private endpoints: Record<string, string> | null;
  private readonly clock: () => number;
  private readonly ca?: string;

  constructor(private readonly o: TokenProviderOptions) {
    if (!o.issuer && !o.tokenUrl) throw new CoordError("oidc_config", "set COORD_OIDC_ISSUER (or COORD_OIDC_TOKEN_URL)");
    if (!o.clientId) throw new CoordError("oidc_config", "set COORD_OIDC_CLIENT_ID");
  ...
  static fromEnv(env: NodeJS.ProcessEnv = process.env): TokenProvider {
    let secret = env.COORD_OIDC_CLIENT_SECRET;
    if (!secret && env.COORD_OIDC_CLIENT_SECRET_FILE) secret = readFileSync(expandUser(env.COORD_OIDC_CLIENT_SECRET_FILE), "utf8").trim();
  ...
  // --- plumbing -----------------------------------------------------------
  private async post(url: string, form: Record<string, string>): Promise<{ status: number; body: Record<string, any> }> {
    let res;
    try {
      res = await httpRequest(new URL(url), "POST", new URLSearchParams(form).toString(),
        { "Content-Type": "application/x-www-form-urlencoded" }, { ca: this.ca, env: this.o.env });
    } catch (e) {
      throw new CoordError("oidc_unreachable", `cannot reach the identity provider: ${(e as Error).message}`);
    }
    try {
  ...
  async endpoint(name: string): Promise<string> {
    if (this.endpoints === null || (!(name in this.endpoints) && this.o.issuer)) {
      ...
      try {
        ...
        throw new CoordError("oidc_unreachable", `OIDC discovery failed at ${url}: ${(e as Error).message}`);
      }
    }
    const url = this.endpoints?.[name];
    if (!url) throw new CoordError("oidc_config", `the identity provider has no ${name}`);
    return url;
  }
  ...
  private async withState<T>(fn: (s: State) => Promise<T>): Promise<T> {
    const dir = join(this.stateFile, "..");
    mkdirSync(dir, { recursive: true });
    try { chmodSync(dir, 0o700); } catch { /* not POSIX */ }
    ...
    for (;;) {
      try {
        ...
        try {
          if (Date.now() - statSync(lock).mtimeMs > LOCK_STALE_MS) unlinkSync(lock); // holder died
        } catch { /* gone meanwhile */ }
        await sleep(50);
      }
    }
    try {
      const state: State = existsSync(this.stateFile) ? JSON.parse(readFileSync(this.stateFile, "utf8")) : {};
  ...
  private store(s: State, tok: Record<string, any>): string {
    s.access_token = tok.access_token;
    s.expires_at = this.clock() + Number(tok.expires_in ?? 60);
    if (tok.refresh_token) s.refresh_token = tok.refresh_token; // Keycloak may rotate it: keep the newest
    return s.access_token!;
  }

  private static clear(s: State) {
  ...
  async token(): Promise<string> {
    return this.withState(async (s) => {
      if (s.access_token && (s.expires_at ?? 0) - EARLY > this.clock()) return s.access_token;
      ...
      if (!s.refresh_token) throw new CoordError("unauthenticated", "not logged in to SSO: run `coord login`");
  ...
  async login(show: (msg: string) => void = (m) => process.stderr.write(m + "\n"),
              wait: (ms: number) => Promise<void> = sleep): Promise<{ logged_in: boolean; refresh_token: boolean }> {
    ...
    while (this.clock() < deadline) {
      ...
      if (tok.error === "slow_down") interval += 5;
      else if (tok.error !== "authorization_pending") throw new CoordError("oidc_login", `device login failed: ${tok.error_description ?? tok.error}`);
    }
